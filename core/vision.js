...
import { clasificarRecibo } from './recibo.js';
...
const PROMPT = `Transcribe todo el texto visible en este recibo o factura, línea por línea, tal como aparece. Incluye el nombre del comercio, la fecha, los artículos y todos los montos, especialmente el total.`;


/**
 * Lee una imagen (Uint8Array o Buffer, JPEG/PNG) y devuelve { datos, textoCrudo, metricas }.
 */
export async function leerRecibo(imagenBytes, onEvento = () => {}) {
  ...
  const ruta = resolve(`data/tmp/doc-${Date.now()}.jpg`);
  ...
  const datos = clasificarRecibo(texto);
  const metricas = {
    fecha: new Date().toISOString(),
    operacion: 'vision-documento',
    modelo: VISION,
