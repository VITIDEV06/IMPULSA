...
const WORKSPACE = 'material-impulso';
const EMBED = {
  nombre: 'EmbeddingGemma-300M', repo: 'unsloth/embeddinggemma-300m-GGUF',
  archivo: 'embeddinggemma-300M-Q8_0.gguf', cuantizacion: 'Q8_0', constanteSdk: 'EMBEDDINGGEMMA_300M_Q8_0'
};
...
function fragmentar(texto, max = 500) {
  ...
  for (const p of parrafos) {
    ...
    let actual = '';
    for (const o of p.split(/(?<=[.;:])\s+(?=[A-ZÁÉÍÓÚÑ0-9])/)) {
      if ((actual + ' ' + o).length > max && actual) { piezas.push(actual); actual = o; }
